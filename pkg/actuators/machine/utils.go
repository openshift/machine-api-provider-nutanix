...
// gpuDeviceInfo holds GPU info for validation (converged client).
type gpuDeviceInfo struct {
	Name     string
	DeviceID *int32
}

// getGPUsForPEConverged returns GPU devices for the given PE cluster using converged client.
func getGPUsForPEConverged(ctx context.Context, client *v4Converged.Client, peUUID string) ([]gpuDeviceInfo, error) {
	profiles, err := client.Clusters.ListClusterPhysicalGPUs(ctx, peUUID)
	if err != nil {
		return nil, err
	}
	out := make([]gpuDeviceInfo, 0, len(profiles))
	for _, p := range profiles {
		if p.PhysicalGpuConfig == nil {
			continue
		}
		cfg := p.PhysicalGpuConfig
		info := gpuDeviceInfo{}
		if cfg.DeviceName != nil {
			info.Name = *cfg.DeviceName
		}
		if cfg.DeviceId != nil {
			id := int32(*cfg.DeviceId)
			info.DeviceID = &id
		}
		out = append(out, info)
	}
...
// getGPUFromListConverged returns a matching GPU from the list for validation.
func getGPUFromListConverged(gpu machinev1.NutanixGPU, devices []gpuDeviceInfo) (*gpuDeviceInfo, error) {
	for _, d := range devices {
		if gpu.Type == machinev1.NutanixGPUIdentifierDeviceID && gpu.DeviceID != nil && d.DeviceID != nil && *gpu.DeviceID == *d.DeviceID {
			return &d, nil
		}
		if gpu.Type == machinev1.NutanixGPUIdentifierName && gpu.Name != nil && *gpu.Name == d.Name {
			return &d, nil
		}
	}
