...
func validateDataDisksConfig(ctx context.Context, mscp *machineScope, fldPath *field.Path) (fldErrs []*field.Error) {
	...
	for _, disk := range disks {
		if GetMibValueOfQuantity(disk.DiskSize) < 1024 {
			fldErrs = append(fldErrs, field.Invalid(fldPath.Child("dataDisks", "diskSize"), disk.DiskSize, "The minimum diskSize is 1Gi bytes"))
		}

		if disk.DeviceProperties != nil {
			deviceType := disk.DeviceProperties.DeviceType
			if deviceType != machinev1.NutanixDiskDeviceTypeDisk && deviceType != machinev1.NutanixDiskDeviceTypeCDROM {
				fldErrs = append(fldErrs, field.Invalid(fldPath.Child("dataDisks", "deviceProperties", "deviceType"), deviceType,
			...
			diskAddr := disk.DeviceProperties.DiskAddress
			if diskAddr.AdapterType != machinev1.NutanixDiskAdapterTypeSCSI &&
				diskAddr.AdapterType != machinev1.NutanixDiskAdapterTypeIDE &&
				diskAddr.AdapterType != machinev1.NutanixDiskAdapterTypePCI &&
				diskAddr.AdapterType != machinev1.NutanixDiskAdapterTypeSATA {
				fldErrs = append(fldErrs, field.Invalid(fldPath.Child("dataDisks", "deviceProperties", "diskAddress", "adapterType"), disk.DeviceProperties.DiskAddress.AdapterType,
					fmt.Sprintf("invalid disk adapter type, the valid types are: %q, %q, %q, %q.", machinev1.NutanixDiskAdapterTypeSCSI, machinev1.NutanixDiskAdapterTypeIDE, machinev1.NutanixDiskAdapterTypePCI, machinev1.NutanixDiskAdapterTypeSATA)))
			}
		}

		if disk.StorageConfig != nil && disk.StorageConfig.StorageContainer != nil {
			storageContainerRef := disk.StorageConfig.StorageContainer
			switch storageContainerRef.Type {
			case machinev1.NutanixStorageContainerReferenceUUID:
				if storageContainerRef.UUID == nil || *storageContainerRef.UUID == "" {
					fldErrs = append(fldErrs, field.Required(fldPath.Child("dataDisks", "storageConfig", "storageContainer", "uuid"), "missing storageContainer uuid"))
				}
			case machinev1.NutanixStorageContainerReferenceName:
				if storageContainerRef.Name == nil || *storageContainerRef.Name == "" {
					fldErrs = append(fldErrs, field.Required(fldPath.Child("dataDisks", "storageConfig", "storageContainer", "name"), "missing storageContainer name"))
				}
			case machinev1.NutanixStorageContainerReferenceURL:
				if storageContainerRef.URL == nil || *storageContainerRef.URL == "" {
					fldErrs = append(fldErrs, field.Required(fldPath.Child("dataDisks", "storageConfig", "storageContainer", "url"), "missing storageContainer url"))
				}
			default:
				errMsg = fmt.Sprintf("invalid storageConfig reference type, the valid types are: %q, %q, %q.", machinev1.NutanixStorageContainerReferenceUUID, machinev1.NutanixStorageContainerReferenceName, machinev1.NutanixStorageContainerReferenceURL)
				fldErrs = append(fldErrs, field.Invalid(fldPath.Child("dataDisks", "storageConfig", "storageContainer", "type"), storageContainerRef.Type, errMsg))
			}
		}
		...
		if disk.DataSource != nil {
			...
			default:
				errMsg := fmt.Sprintf("invalid disk dataSource identifier type, the valid types are: %q, %q.", configv1.NutanixIdentifierUUID, configv1.NutanixIdentifierName)
	...
	for _, gpu := range gpus {
		...
		case machinev1.NutanixGPUIdentifierDeviceID:
			if gpu.DeviceID == nil {
				...
				_, err := GetGPUFromList(ctx, mscp.nutanixClient, gpu, peGPUs)
				if err != nil {
		...
		default:
			errMsg = fmt.Sprintf("invalid gpu identifier type, the valid types are: %q, %q.", machinev1.NutanixGPUIdentifierDeviceID, machinev1.NutanixGPUIdentifierName)
		...
		for _, dataDisk := range mscp.providerSpecValidated.DataDisks {
			...
			if dataDisk.DeviceProperties != nil {
				vmDisk.DeviceProperties = &nutanixClientV3.VMDiskDeviceProperties{
					DeviceType: utils.StringPtr(string(dataDisk.DeviceProperties.DeviceType)),
					DiskAddress: &nutanixClientV3.DiskAddress{
						AdapterType: utils.StringPtr(string(dataDisk.DeviceProperties.DiskAddress.AdapterType)),
						DeviceIndex: utils.Int64Ptr(dataDisk.DeviceProperties.DiskAddress.DeviceIndex),
					},
				}
			}
			...
			if dataDisk.StorageConfig != nil {
				...
				if dataDisk.StorageConfig.FlashMode {
					vmDisk.StorageConfig.FlashMode = "ON"
				}

				if dataDisk.StorageConfig.StorageContainer != nil {
					//  vmDisk StorageContainerReference: either one of (kind, uuid) or url needs to be specified.
					...
					case machinev1.NutanixStorageContainerReferenceURL:
						vmDisk.StorageConfig.StorageContainerReference = &nutanixClientV3.StorageContainerReference{
							URL: *dataDisk.StorageConfig.StorageContainer.URL,
						}
					}
				}
			}
