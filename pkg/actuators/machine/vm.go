...
// listVMNics fetches NIC details for a VM using the converged client's
// ListNicsByVmId API. The VM GET response may not include NIC IP details
// (learnedIpAddresses); this dedicated endpoint reliably returns NIC
...
func getPrismCentralCluster(ctx context.Context, client *v4Converged.Client) (*clusterModels.Cluster, error) {
	...
	// Prefer cluster with PRISM_CENTRAL function; fall back to the sole cluster if only one exists.
	var found *clusterModels.Cluster
	var fallback *clusterModels.Cluster
	for i := range clusters {
		if len(clusters) == 1 && clusters[i].ExtId != nil {
			fallback = &clusters[i]
		}
